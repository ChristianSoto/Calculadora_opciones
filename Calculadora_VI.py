...
window=tk.Tk()
window.title('Calculadora opciones')
...
def cargardata():
    ...
    def ImpliedVol(S0, K, T, r, C0, sigma_est, it=100):
        ...
        op_delta=delta(opcion.lower(), S0, K, T, r, sigma_est)
        op_gamma=gamma(opcion.lower(),S0,K,T,r,sigma_est)
        op_vega=vega(opcion.lower(),S0,K,T,r,sigma_est)
        op_thetha=theta(opcion.lower(),S0,K,T,r,sigma_est)
        op_rho=rho(opcion.lower(),S0,K,T,r,sigma_est)
        result["text"]= f'La volatilidad implícita es de {round(sigma_est*100,2)}% \n Delta: {round(op_delta,2)} \n Gamma: {round(op_gamma,2)} \n Vega: {round(op_vega,2)}\n Theta: {round(op_thetha,2)} ' \
                        f'\n Rho: {round(op_rho,2)}'
